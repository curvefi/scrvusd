...
def test_default_behavior(rewards_handler, minimum_weight, requested_weight, curve_dao):
    rewards_handler.eval(f"twa._store_snapshot({requested_weight})")
    ...
    assert rewards_handler.weight() == max(minimum_weight, requested_weight)
